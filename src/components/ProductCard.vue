...
<div class="product">
    <h2>{{productName}}s</h2>
    <div> 
        <img width="200" :src="productTypes[index].img" />
        <div class="color-container" v-for="(color, i) in colors()" :style="{backgroundColor:color}" :key="i">
            <div @mouseover="setID(type.id)" class="main-color" v-for="(type, i) in productTypes" v-show="type.colorMain === color" :style="{backgroundColor:type.colorMain}" :key="i"> 
